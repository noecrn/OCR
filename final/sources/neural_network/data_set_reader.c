...
*/
void data(int numTraining, int numTesting, double **training, 
double *expectedTrain, double **testing, double *expectedTest)
{
	...
	struct Image *img = initImage(line);
        ...
        if (training[i] == NULL) {
            fprintf(stderr, 
            "Erreur d'allocation de mémoire pour le tableau %d.\n", i);
...
void freeData(int numTraining, int numTesting, 
double **training, double *expectedTrain, 
double **testing, double *expectedTest)
{
