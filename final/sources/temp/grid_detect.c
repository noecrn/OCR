...
void rotate(double angle,char * inputPath,int color)
{
    ...
    SDL_RenderCopyEx(renderer, SDL_CreateTextureFromSurface
    (renderer, imageSurface), NULL, &dstRect, angle, NULL, SDL_FLIP_NONE);
        ...
        int *yf3,int *lf0,int *lf1,int *lf2,int *lf3)
{
    ...
    for (int y = 1; y < h-1 ; y++)
    {
            for (int x = 1; x < w-1 ; x++)
            {
                ...
                SDL_GetRGB(*(Uint32 *)
                        ...
                        Surface->format->BytesPerPixel), Surface->format, &r, 
                        &g, &b);
    ...
    void angleCheck()
    {
        ...
        if (angle>4||angle<-4)
        {
            ...
            SDL_Surface *newImageFiltered = SDL_CreateRGBSurface(0, w0, h0, 
            32, 0, 0, 0, 0);

            SDL_Surface *newImageOriginal = SDL_CreateRGBSurface(0, w0, h0, 
            32, 0, 0, 0, 0);
            for (int y = 1; y < h0-1 ; y++)
                {
                        for (int x = 1; x < w0-1 ; x++)
                        {
                            ...
                            SDL_GetRGB(*(Uint32 *)
                                    ((Uint8 *)tempFiltered->pixels + (y+yy) 
                                    * tempFiltered->pitch + (x+xx) *
                            ...
                            *(Uint32 *)
                            ...
                            SDL_GetRGB(*(Uint32 *)
                                    ((Uint8 *)tempOriginal->pixels + (y+yy) * 
                                    tempOriginal->pitch + (x+xx) *
