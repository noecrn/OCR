...
void resizeImageWithoutRatio(SDL_Surface* originalSurface) {
    ...
    SDL_Surface * resizedSurface = SDL_CreateRGBSurfaceWithFormat(0, 28, 28,
                                                                originalSurface
                                                                ->format->
                                                                BitsPerPixel,
                                                                originalSurface
...
void extract(int x[],int y[],int * t)
{
    ...
    line (&slope,&y_intercept,(float) x[0]*ratioX,(float) y[0]*ratioY,(float) 
    x[1]*ratioX,(float) y[1]*ratioY);
    coordonates[0][0]=slope;
    coordonates[0][1]=y_intercept;
    line (&slope,&y_intercept,(float) x[0]*ratioX,(float) y[0]*ratioY,(float) 
    x[3]*ratioX,(float) y[3]*ratioY);
    coordonates[10][0]=slope;
    coordonates[10][1]=y_intercept;   
    line (&slope,&y_intercept,(float) x[2]*ratioX,(float) y[2]*ratioY,(float) 
    x[1]*ratioX,(float) y[1]*ratioY);
    ...
    for (int xx=0; xx<9;xx++)
    {
        for (int yy=10; yy<19;yy++)
        {
            ...
            aux(&xd,&yd,coordonates[xx][0],coordonates[yy][0],coordonates[xx][1],
            coordonates[yy][1]);
            ...
            SDL_Surface *Number = 
            SDL_CreateRGBSurface(0, longX+1, longY+1, 32, 0, 0, 0, 0);
            for (int x = xd; x < xa ; x++)
            {
                for (int y = yd; y < ya ; y++)
                {
                    ...
                    if (x-xd>-1 && x-xd<(longX) && y-yd>-1 && y-yd<longY-1){
                        *(Uint32 *)
                            ((Uint8 *)Number->pixels + (int)((y-yd)) * 
                            Number->pitch + (int)((x-xd)) *
