...
void grid (int *xf0,int *yf0,int *xf1,int *yf1,int *xf2,int *yf2,int *xf3,
        int *yf3,int *lf0,int *lf1,int *lf2,int *lf3,char * imagePath)
{
    resizeImage(imagePath, "temp.png", 700);
    ...
    void angleCheck()
    {
        ...
        if (angle>4||angle<-4)
        {
            ...
            int xx=(w1-w0)/2;
            int yy=(h1-h0)/2;
            SDL_Surface *newImage = SDL_CreateRGBSurface(0, w0, h0, 32, 0, 0, 0, 0);
            for (int y = 1; y < h0-1 ; y++)
                {
                        for (int x = 1; x < w0-1 ; x++)
                        {
                            ...
                            SDL_GetRGB(*(Uint32 *)
                                    ((Uint8 *)Surface->pixels + (y+yy) * Surface->pitch + (x+xx) *
                                    Surface->format->BytesPerPixel), Surface->format, &r, &g, &b);

                            *(Uint32 *)
                                ((Uint8 *)newImage->pixels + y * newImage->pitch + x *
                                newImage->format->BytesPerPixel)
                                = SDL_MapRGB(newImage->format, r, g, b);
                        }
                }
            IMG_SavePNG(newImage, "temp.png");
            ...
            grid (&x0,&y0,&x1,&y1,&x2,&y2,&x3,&y3,
                    &l0,&l1,&l2,&l3,"temp.png");
        }
    }

    void parallelCheck()
    {
        void aux (int *xx,int *yy,float s0,float s1,float y0_intercept,float y1_intercept)    // calcul les coordonnées d'intersection de deux droites
        {
        ...
        void calculateCoordonates()
        {
            ...
            {
                aux(&x,&y,coordonates[0][0],coordonates[10][0],coordonates[0][1],coordonates[10][1]);
                x0=x;
                y0=y;
            }
            if (coordonates[0][0]!=-999 && coordonates[19][0]!=-999) 
            {
                aux(&x,&y,coordonates[0][0],coordonates[19][0],coordonates[0][1],coordonates[19][1]);
                x1=x;
                y1=y;
            }
            if (coordonates[9][0]!=-999 && coordonates[19][0]!=-999) 
            {
                aux(&x,&y,coordonates[9][0],coordonates[19][0],coordonates[9][1],coordonates[19][1]);
                x2=x;
                y2=y;
            }
            if (coordonates[9][0]!=-999 && coordonates[10][0]!=-999) 
            {
                aux(&x,&y,coordonates[9][0],coordonates[10][0],coordonates[9][1],coordonates[10][1]);
                x3=x;
                y3=y;
            }
